...
/**
 * クライアント証明書有効性検証のバリデーションチェック
 */
@Middleware({ type: 'before' })
export default class ValidRequestValidator implements ExpressMiddlewareInterface {
    async use (request: Request, response: Response, next: NextFunction) {
        ...
        if (this.isEmpty(body['serialNo'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(Message.validation.isString, 'serialNo'), ResponseCode.BAD_REQUEST);
        }
        if (this.isEmpty(body['fingerPrint'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(Message.validation.isString, 'fingerPrint'), ResponseCode.BAD_REQUEST);
        }
