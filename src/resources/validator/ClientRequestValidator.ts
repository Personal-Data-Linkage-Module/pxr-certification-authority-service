...
/**
 * クライアント証明書のバリデーションチェック
 */
@Middleware({ type: 'before' })
export default class ClientRequestValidator implements ExpressMiddlewareInterface {
    async use (request: Request, response: Response, next: NextFunction) {
        ...
        if (this.isUndefined(body['block']) || this.isNull(body['block'])) {
            // エラーレスポンスを返す
        ...
        if (this.isUndefined(body['actor']) || this.isNull(body['actor'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(Message.validation.isDefined, 'actor'), ResponseCode.BAD_REQUEST);
        }
        if (this.isUndefined(body['actor']['value']) || this.isNull(body['actor']['value'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(Message.validation.isDefined, 'actor.value'), ResponseCode.BAD_REQUEST);
        }
        if (this.isUndefined(body['actor']['ver']) || this.isNull(body['actor']['ver'])) {
            // エラーレスポンスを返す
        ...
        if (!this.isNumber(body['actor']['value'])) {
            // エラーレスポンスを返す
